...
int main(void)
{
    ...
    T_PsdkHalUartHandler halUartHandler = {//串口接口结构体声明
        .UartInit = Hal_UartInit,
        .UartReadData = Hal_UartReadData,
        .UartWriteData = Hal_UartSendData,
    };

    T_PsdkHalNetWorkHandler halNetWorkHandler = {//网口接口结构体声明
        .NetWorkConfig = HalNetWork_Config,
    };

    T_PsdkOsalHandler osalHandler = {//操作系统抽象层接口结构体声明
        .Malloc = Osal_Malloc,
        .Free = Osal_Free,
    ...
    if (PsdkPlatform_RegHalUartHandler(&halUartHandler) != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {//uart跨平台移植注册
        printf("psdk register hal uart handler error");
        return PSDK_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (PsdkPlatform_RegHalNetworkHandler(&halNetWorkHandler) != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {//network跨平台移植注册
        printf("psdk register hal network handler error");
        return PSDK_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (PsdkPlatform_RegOsalHandler(&osalHandler) != PSDK_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {//osal操作系统抽象层跨平台移植注册
